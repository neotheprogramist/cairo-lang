...
def parse_proof(identifiers: IdentifierManager, proof_json: dict):
    ...
    # Parse JSON.
    public_input: PublicInput = PublicInput.Schema().load(
        proof_json["public_input"])
    ...
    (original_commitment_hash,) = annotations(
        "STARK/Original/Commit on Trace", "Hash")
    ...
    fri_layers_commitments = annotations(
        "STARK/FRI/Commitment/Layer [0-9]+", "Hash")
    ...
    for i in range(1, n_fri_layers):
        leaves = annotations(
            f"STARK/FRI/Decommitment/Layer {i}", "Field Element")
        authentications = annotations(
            f"STARK/FRI/Decommitment/Layer {i}", "Hash")
    ...
    module_name = f"starkware.cairo.stark_verifier.air.layouts.{
        public_input.layout}.autogenerated"
    ...
    log_trace_domain_size = safe_log2(
        effective_component_height * public_input.n_steps)
    ...
    assert len(layer_log_sizes) == n_fri_layers + 1
